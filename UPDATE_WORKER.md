...
docker build --no-cache --platform linux/amd64 -t jingyuyan19/watermark-worker:v1.25.1 -t jingyuyan19/watermark-worker:latest .

# Push to Docker Hub
docker push jingyuyan19/watermark-worker:v1.25.1
docker push jingyuyan19/watermark-worker:latest
...
5.  Change `jingyuyan19/watermark-worker:latest` to `jingyuyan19/watermark-worker:v1.25.1`.
6.  Click **Save**.
7.  The next cold start will pull `v1.25.1`. This effectively "locks" your worker to that version.
