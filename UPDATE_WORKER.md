...
docker build --no-cache --platform linux/amd64 -t jingyuyan19/watermark-worker:v1.27.1 -t jingyuyan19/watermark-worker:latest .

# Push to Docker Hub
docker push jingyuyan19/watermark-worker:v1.27.1
docker push jingyuyan19/watermark-worker:latest
...
5.  Change `jingyuyan19/watermark-worker:latest` to `jingyuyan19/watermark-worker:v1.27.1`.
6.  Click **Save**.
7.  The next cold start will pull `v1.26.9`. This effectively "locks" your worker to that version.

## 21. The "Safety" Hotfix (v1.27.1)
**Goal:** Fix detection for dynamic watermarks (Sora 2).
**Changes:**
1.  **Reverted Stroboscopic Detection**: Now processes **every frame** again, but still uses the optimized video loading (speedup over v1.26.9 maintained).
2.  **Reason**: Watermarks jump randomly in newer videos, making interpolation unsafe.
3.  **Island-Aware Budgeting**: Maintained (Optimized cleaning is still active).
...
**Changes:**
1.  **Manual Resizing**: Implemented `cv2.resize` to 640px BEFORE passing images to YOLO. This avoids massive CPU overhead when processing 4K frames loop-style.
2.  **Logic**: Input frames are resized, inference runs on 640px tensor, output boxes are scaled back.

## 18. The "Hotfix" Update (v1.26.8)
**Goal:** Fix Syntax Error.
**Changes:**
1.  **Syntax Fix**: Removed accidental double `if line:` check in `core.py` that caused startup crash.
2.  **Cleanup**: Removed duplicate comments in `watermark_detector.py`.

## 17. The "Force CUDA" Update (v1.26.7)
**Goal:** Fix Slow Detection & Missing Logs.
**Changes:**
1.  **Log Visibility**: `core.py` now checks for debug lines *before* progress parsing, ensuring they are logged even if lines are mixed.
2.  **Force CUDA**: `watermark_detector.py` now explicitly passes `device=...` to `model.predict()`, leaving no room for CPU fallback.
